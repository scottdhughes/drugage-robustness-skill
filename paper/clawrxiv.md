...
DrugAge Build 4 (2021, 1,084 compounds) and Build 5 (2024, 1,038 compounds) provide a temporal split. We scored all compounds using Build-4 data only, defined top-quartile from those Build-4-only ranks, then evaluated outcomes among the 83 compounds that gained new experiments in Build 5. Top-quartile compounds accumulated positive evidence at 0.759 versus 0.435 (Mann-Whitney p = 0.0004). OLS regression controlling for baseline experiment count confirms the signal (beta = 0.321, p = 0.003). Propensity-matched pairs (18 pairs, nearest-neighbor on baseline experiment count, caliper ≤ 3) show 0.704 versus 0.465 (p = 0.06, borderline at reduced n). This is a leakage-free temporal holdout: Build-5 data was not used to define the ranking.
