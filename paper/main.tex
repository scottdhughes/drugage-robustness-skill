...
\begin{abstract}
We present an evidence-robustness index for longevity interventions in DrugAge that ranks compounds by cross-species consistency rather than raw lifespan effect. The index separates from a 1{,}000-permutation species-stratified null ($z = 4.42$, $p = 0.001$) and had the highest point-estimate AUROC on a leakage-controlled ITP holdout ($0.847$, 95\% CI $[0.614, 1.000]$, $n = 17$) among prespecified baselines. A leakage-free out-of-time validation (Build-4-only scoring $\rightarrow$ Build-5 outcomes) shows top-quartile compounds accumulate significantly more positive evidence ($0.759$ vs.\ $0.435$, $p = 0.0004$), surviving after controlling for baseline experiment count ($\beta = 0.321$, $p = 0.003$). An empirical Bayes beta-binomial model provides compound-level credible intervals ($\tau = 0.67$ vs.\ heuristic). The ranking is stable under weight perturbation (median $\tau = 0.77$), cap perturbation (80.6\% yield $\tau > 0.80$), and PMID-clustered bootstrap ($\tau = 0.76$, CI $[0.67, 0.83]$). All metrics are formally defined. Code: \texttt{github.com/scottdhughes/drugage-robustness-skill}.
\end{abstract}
...
The pipeline classified 48 compounds as robust, 174 as promising, 435 as thin evidence, and 381 as conflicted. The robust-compound count (48) exceeds the permutation null mean (29.9) at $z = 4.42$, $p = 0.001$.

\subsection{ITP Cross-Validation}

Of 54 ITP-tested compounds, 53 matched DrugAge. All 12 ITP-negative compounds landed in the conflicted tier (Fisher's exact $p = 0.0036$). Under a clean holdout (all 164 ITP rows removed before scoring), 7/8 ITP-positive compounds remained in the top quartile. The robustness score had the highest point-estimate AUROC ($0.847$, 95\% CI $[0.614, 1.000]$) on the holdout, followed by experiment count ($0.833$), trimmed mean ($0.792$), and median effect ($0.771$). The CI excludes chance but is wide ($n = 17$); we treat this as preliminary evidence of discriminative value, not a definitive superiority claim.

\subsection{Out-of-Time Validation: Build 4 to Build 5}

DrugAge Build~4 (2021, 1{,}084 compounds) and Build~5 (2024, 1{,}038 compounds) provide a temporal split. We scored all compounds using Build-4 data only, defined top-quartile membership from those Build-4-only ranks, then evaluated outcomes among the 83 compounds that gained new experiments in Build~5. Top-quartile compounds (as ranked by Build-4 evidence alone) accumulated positive evidence at $0.759$ versus $0.435$ for lower-ranked compounds (Mann--Whitney $p = 0.0004$, $n_{\text{top}} = 35$, $n_{\text{bottom}} = 48$). An OLS regression controlling for baseline experiment count confirms the signal ($\beta = 0.321$, $p = 0.003$; the baseline-count coefficient is near zero, $p = 0.97$). Propensity-matched pairs (18 pairs, nearest-neighbor on baseline experiment count within caliper $\leq 3$) show 0.704 versus 0.465 ($p = 0.06$, borderline at reduced $n$). This is a leakage-free temporal holdout: Build-5 data was not used to define the ranking.
...
\begin{tabular}{lr}
\toprule
Ranking method & Holdout AUROC \\
\midrule
Sign consistency alone & 0.500 \\
Species $\times$ sign cons.\ ($n_{\text{sp}} \cdot \text{sc} \cdot \mathbf{1}[\tilde{x}>0]$) & 0.708 \\
Median effect & 0.771 \\
Trimmed-mean effect & 0.792 \\
Experiment count & 0.833 \\
Robustness $R$ & \textbf{0.847} \\
\bottomrule
\end{tabular}
\caption{ITP-holdout AUROC (8 positive, 9 negative, all ITP rows excluded before scoring). The species $\times$ sign-consistency baseline is defined as $n_{\text{species}} \cdot \text{sign\_consistency}$ for compounds with positive median effect, zero otherwise.}
\label{tab:baselines}
\end{table}

\section{Sensitivity and Robustness Diagnostics}

\textbf{Weight and cap stability.} 200 Dirichlet-sampled weight vectors yield median Kendall $\tau = 0.77$ versus the canonical ranking; 80.6\% of 160 breadth-cap combinations yield $\tau > 0.80$. The continuous score is moderately stable to weight and cap perturbation, while robust-tier membership is most sensitive to the minimum-species gate.
...
\textbf{Structure-matched null.} An exploratory null shuffling compound identity within species attenuates the signal to $z = 1.26$ ($p = 0.13$), confirming part of the robust-tier enrichment is structural rather than biological.

\textbf{Unseen-species validation.} Among 50 compounds that gained experiments in species not represented in their Build-4 evidence, top-quartile compounds showed a new-species positive rate of 0.776 versus 0.500 for lower-ranked compounds (Mann--Whitney $p = 0.024$). This confirms cross-species generalization: top-ranked compounds show positive effects even in previously untested species.

\textbf{Pure-compound-only sensitivity.} Of 48 robust-tier compounds, 40 are pure compounds and 8 are crude extracts/nutraceuticals. After excluding all extracts, the pure-compound top-10 is identical to the canonical top-10 minus Apple extract (rank 2). The ranking is not driven by extract heterogeneity.

\textbf{Publication bias.} 70.1\% of DrugAge's 3{,}372 experiments report positive lifespan change. This positivity skew is a structural feature of the database that inflates sign-consistency scores. The hidden-negative injection grid quantifies the resulting fragility: at 30\% injection, 33\% of the robust tier is lost. The index cannot distinguish genuine geroprotective signal from systematic underreporting of negative results.
...
This evidence-robustness index ranks longevity claims by analytical consistency, separates from a permutation null ($z = 4.42$), and predicts future evidence accumulation across database builds ($p = 0.0004$ on a leakage-free Build-4-only holdout, controlled for attention). It had the highest ITP concordance among baselines (AUROC $= 0.847$ on clean holdout) and is stable under weight, cap, threshold, species-weighting, and PMID-clustered perturbations. The index measures evidence robustness inside DrugAge, not biological efficacy.
